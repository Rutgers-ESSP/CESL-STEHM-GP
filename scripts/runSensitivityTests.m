...
clear sensf senssd sensnames senscoords sensthet;
    ...
    for doexcl=0:1
        ...
        if length(trainsub)>0
            ...
            if reoptimize
                ...
                startcomact=thetTGG{jj}(end);
                ms.subfixed = 1:length(ms.thet0);
                [dothet]=OptimizeHoloceneCovariance(wdataset,ms,[3.0],trainfirsttime(end),trainrange(end),1e6,startcompact);  
            else
                dothet=thetTGG{jj};
            end
            
            
            [wf,wsd,wV]=RegressHoloceneDataSets(wdataset2,wtestsitedef,modelspec(trainspecs(jj)),dothet,trainsub,noiseMasks(1,:),wtestt,refyear,collinear);
...
fprintf(fid,'Site\tLat\tLong\tN\tInclusive f (mm/y)\t1s\tpercent var reduction\tpercent var reduction/N\tExclusive f (mm/y)\t1s\tpercent var increase\tpercent var increase/N\thyperparameters\n');
