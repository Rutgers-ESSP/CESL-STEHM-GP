...
%
% Last updated by Robert Kopp, robert-dot-kopp-at-rutgers-dot-edu, Sat Oct 25 22:57:12 EDT 2014
...
function logp=logprobNI(x0,y0,dx0,dy0,traincvaug,cvfunc,thet,spacex)

	if min(size(dy0))==1
            [dK,df,d2f,yoffset] = GPRdx(x0,y0,dx0,diag(dy0.^2)+traincvaug(thet),@(x1,x2,r1,r2) cvfunc(x1,x2,thet,r1,r2),2,spacex);
	    logp = logprob(y0-yoffset,@(theta) cvfunc(x0,x0,theta,spacex,spacex)+diag(dy0.^2)+diag(dK)+traincvaug(theta),thet,[]);
	else
            [dK,df,d2f,yoffset] = GPRdx(x0,y0,dx0,dy0+traincvaug(thet),@(x1,x2,r1,r2) cvfunc(x1,x2,thet,r1,r2),2,spacex);
