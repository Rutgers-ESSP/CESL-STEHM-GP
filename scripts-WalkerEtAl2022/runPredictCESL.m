...
for iii=1:length(regressparams)
    ...
    noiseMasks = ones(7,length(thetTGG{jj}));
    ...
    noiseMasks(7,[ wmodelspec.subampoffset wmodelspec.subampnoise wmodelspec.subamplinear wmodelspec.subampglobal]  )=0;
    noiseMasklabels={'denoised','lin','nonlin','global','reg-nonlin','local','regloc-nonlin'};
    ...
    save(savefile,'datasets','modelspec','f2s','sd2s','V2s', ...
         'testlocs','logp','testsitedef','trainspecs','thetTGG','ICE5G','noiseMasks','testt','refyear', '-v7.3')
   ...
   if dosldecomp
        ...
        makeplots_sldecomp(wdataset,f2s{iii},sd2s{iii},V2s{iii},testlocs{iii},labl,[1 2 3 4 5 6],0);
    end
    ...
    % Run probability model with 60 yr rates
    runPunusual;
