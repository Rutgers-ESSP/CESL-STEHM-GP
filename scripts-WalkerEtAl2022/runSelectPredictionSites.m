...
% 
% Last updated by Robert Kopp, robert-dot-kopp-at-rutgers-dot-edu, 2019-05-15 09:50:07 -0400
...
%% ADD ARBITARY SITE FAR AWAY FROM DATA
% we will use the lat/long of Seoul 

testsitedef.sites(end+1,:)=mean([9999999 37.53 127.05],1); %this is for
%Seoul
%wdist = dDist(testsitedef.sites(end,2:3),wdataset.sitecoords);
%avail=avail.*(wdist>.2); % keep sites at least .2 degrees apart
testsitedef.names2={testsitedef.names2{:}, 'Seoul-NODATA'};

testsitedef.names={testsitedef.names{:}, 'Seoul-NODATA'};

testsitedef.firstage = [testsitedef.firstage min(oldest)];
testsitedef.oldest = [testsitedef.oldest min(oldest)];
testsitedef.youngest = [testsitedef.youngest 2014];
...
testsitedef.firstage=min(0,testsitedef.firstage);
