...
cvfunc.I = @(dt1t2,ad,thetas,fp1fp2,onoff) kMat3(dt1t2,thetas(1:2)) .* (onoff(1) + onoff(2)*fp1fp2);
...
ddcvfunc.I = @(dt1t2,ad,thetas,fp1fp2,onoff) kMat3dd(dt1t2,thetas(1:2)) .* (onoff(1) + onoff(2)*fp1fp2);
ddcvfunc.W = 0;
ddcvfunc.f0 = 0;
...
modelspec(1).subonoff = 10:11;
modelspec(1).subfixed=modelspec(1).subonoff; % fix GIS switches
modelspec(1).sublength=[4 7];

modelspec(1).subamp = [1 3 5 8 12 13];
modelspec(1).subamplinear = [3];
modelspec(1).subampglobal = [1];
modelspec(1).subampoffset = [13];
...
modelspec(1).subHPoffset = [13];
modelspec(1).subHPGIS = [8 9 10 11];
modelspec(1).subHPregmat = [5 6 7];
modelspec(1).subHPnoise = [12];
